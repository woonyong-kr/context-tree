...
6. The current-note node and direct-link nodes share the primary force simulation and can all be dragged after deliberate screen-space movement. A press remains an ordinary activation path without pointer capture; only crossing the drag threshold captures the pointer. A successful drop pins the node at that session-only position; pointer cancellation or app focus loss restores the pre-drag position. Clicking the current-note node opens its resolved canonical `parent` when one exists.
