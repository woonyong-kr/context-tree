...
10. Outline view projects the full direct-link set in authored order. Graph view renders at most 120 direct nodes. A hover preview shows at most 8 next routes in a compact sidebar, 16 in a medium pane, and 48 in a wide pane; the accessibility status reports shown and total counts, while activating the source opens its complete graph. Direct-route omissions switch to the complete Outline view.
