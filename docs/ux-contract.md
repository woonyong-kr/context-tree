...
6. The current-note node and direct-link nodes share the primary force simulation and can all be dragged after deliberate screen-space movement. Each direct node has one padded interaction owner for hover, activation, and drag; its inner native button supplies keyboard semantics. A short press activates exactly once on pointer release without propagation blocking or pointer capture, while only crossing the drag threshold captures the pointer and suppresses activation. Keyboard click and pointer release call the same activation method. Background pan excludes node owners by target. A successful drop pins the node at that session-only position; pointer cancellation or app focus loss restores the pre-drag position. Activating the current-note node opens its resolved canonical `parent` when one exists.
