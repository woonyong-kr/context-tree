...
1. 일반 카드 클릭은 **제자리에서** Reading 카드를 열거나 닫는다. 클릭은 카메라를 이동시키지 않는다. Reading 또는 Source가 열린 동안 현재 카드 좌표 전체는 하나의 공간 문맥으로 보존되며, 카드 크기 측정이 force layout을 다시 깨우지 않는다. 새로 펼친 이웃만 기존 좌표 밖에서 배치될 수 있다.
...
1. 시각 언어는 **문서 우선의 조용한 작업면**이다. Notion과 같은 문서 도구의 원칙을 따르되 특정 제품의 외형을 복제하지 않는다. 카드와 도구는 Obsidian 테마의 중성 배경·텍스트 변수를 사용하고, 강한 색상·큰 그림자·항상 보이는 버튼 테두리로 내용과 경쟁하지 않는다. 강조색은 검색 결과, 선택된 관계, keyboard focus처럼 의미가 있는 상태에만 사용한다.
...
3. 캔버스는 장식 격자 없는 단색 작업면이다. 관계선은 카드 본문보다 한 단계 낮은 대비를 사용하며, 선택하거나 직접 연결을 살펴볼 때만 선명해진다. 우하단 도구 모음과 검색 패널은 카드와 같은 중성 surface·radius·shadow 체계를 공유한다.
...
10. 카드의 직접 동작과 더보기는 같은 hover·keyboard focus 상태에서 드러난다. 낮은 그래프 배율에서도 카드 동작은 제한된 역배율을 적용해 클릭 가능한 크기를 유지하되 카드보다 커지지 않는다. 원본 편집기 동작은 Reading·Source 전환으로 다른 아이콘이 생겨도 더보기 바로 왼쪽의 같은 슬롯을 유지한다.
...
10. **시각 계층:** 기본 테마와 설치된 사용자 테마에서 compact·Reading·Source 카드를
   각각 확인한다. compact 카드는 평상시 장식 경계가 없고 관계선은 중성 저대비이며, 검색·관계 선택·keyboard
   focus만 강조색을 사용한다. 카드 hover와 Source focus는 폭·높이·좌표를 바꾸지 않고,
   카드 동작은 하나의 hover surface로 보이며 제목과 겹치지 않는다. 우하단 도구 모음,
