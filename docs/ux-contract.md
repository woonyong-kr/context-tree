...
3. Only resolved, non-embedded outgoing wikilinks are shown. Dot-prefixed internal roots, private/source, generated/archive, and retired/superseded destinations are excluded from routes and parent navigation. Ordinary root folders such as `brain` and `inbox` are never assigned special meaning.
