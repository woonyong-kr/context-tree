...
7. For a direct node, confirm a single click opens it without a second disclosure button or `빠른 이동`/`하위 노드` caption. Hover intent must expose outgoing keywords without moving the source, keyboard focus must expose the same preview immediately, and Enter must open the note. Preview nodes and edges must reveal and collapse together without detached endpoints.
